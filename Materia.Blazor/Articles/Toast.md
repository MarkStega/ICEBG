...
The toast service is an added extra, styled to fit in with Materia.Blazor, but outside the Material Theme specification. Our implementation is a revised port of
...
listed on the [IMBToastService class api documentation page](xref:Materia.Blazor.IMBToastService). You will also need 
to place the 
...
call a toast using its [ShowToast() method](xref:Materia.Blazor.IMBToastService#methods).
