...
public abstract class InputComponent<T> : ComponentFoundation
{
    ...
    /// <summary>
    /// Gets or sets the value of the component. To be used by Materia.Blazor components for binding to
        ...
        set
        {
            ...
            if (!EqualityComparer<T>.Default.Equals(value, _componentValue))
            {
                ...
                if (EditContext != null && !IgnoreFormField)
                {
                    ...
                    {
                        throw new Exception("Materia.Blazor: ValueExpression must be defined for a field contained in an EditForm");
                    }
                    else
                    {
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        //if (EditContext != null && IgnoreFormField)
        //{
        //    LoggingService.LogWarning($"{GetType()} is in a form but has EditContext features disabled because it is considered a valid Materia.Blazor form field type");
        //}
    }
    ...
    // Would like to use <inheritdoc/> however DocFX cannot resolve to references outside Materia.Blazor.
    //
    ...
    ///// <summary>
    ///// Materia.Blazor components descending from InputComponent _*must not*_ override ShouldRender().
    ///// </summary>
